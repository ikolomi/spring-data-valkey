...
/**
 * Provider for Valkey Cluster node resources using Valkey-Glide.
 *
 * @author Ilya Kolomin
 * @since 2.0
 */
public class ValkeyGlideClusterNodeResourceProvider {
    ...
    public <T> T getResourceForNode(ValkeyClusterNode node, Function<ValkeyClusterNode, T> resourceFactory) {
    ...
    @Nullable
    @SuppressWarnings("unchecked")
    public <T> T getResourceForNode(ValkeyClusterNode node) {
        Assert.notNull(node, "ValkeyClusterNode must not be null!");
     ...
     * @param node The cluster node.
     */
    public void closeResourceForNode(ValkeyClusterNode node) {
        Assert.notNull(node, "ValkeyClusterNode must not be null!");
     ...
     * @return The key.
     */
    private String createNodeKey(ValkeyClusterNode node) {
        return node.getId();
    }
