...
import org.springframework.data.valkey.connection.convert.TransactionResultConverter;
import org.springframework.data.valkey.connection.lettuce.LettuceConnectionProvider.TargetAware;
import org.springframework.data.valkey.connection.lettuce.LettuceResult.LettuceResultBuilder;
import org.springframework.data.valkey.connection.lettuce.LettuceResult.LettuceStatusResult;
import org.springframework.data.valkey.core.Cursor.CursorId;
import org.springframework.data.valkey.core.ValkeyCommand;
...
public class LettuceConnection extends AbstractValkeyConnection {
	...
	@Override
	public org.springframework.data.valkey.connection.ValkeyCommands commands() {
		return this;
	}
	...
	public Object execute(String command, @Nullable CommandOutput commandOutputTypeHint, byte[]... args) {
		...
		Command valkeyCommand = new Command(commandType, expectedOutput, commandArguments);

		return invoke().just(RedisClusterAsyncCommands::dispatch, valkeyCommand.getType(), valkeyCommand.getOutput(),
				valkeyCommand.getArgs());
	}
	...
	private RedisCommands<byte[], byte[]> getDedicatedValkeyCommands() {
	...
	private void validateCommand(ProtocolKeyword command, @Nullable byte[]... args) {
		...
		if (!ValkeyCommand.UNKNOWN.equals(valkeyCommand) && valkeyCommand.requiresArguments()) {
			try {
				valkeyCommand.validateArgumentCount(args != null ? args.length : 0);
