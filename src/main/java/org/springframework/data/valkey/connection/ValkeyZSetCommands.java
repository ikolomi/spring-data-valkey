...
public interface ValkeyZSetCommands {
	 ...
	 * @author Christoph Strobl
	 * @since 1.6
	 * @deprecated since 3.0, use {@link org.springframework.data.valkey.connection.Limit} instead.
	 */
	@Deprecated
	class Limit extends org.springframework.data.valkey.connection.Limit {
	...
	default Set<byte[]> zRangeByScore(byte[] key, double min, double max, long offset, long count) {
		return zRangeByScore(key, org.springframework.data.domain.Range.closed(min, max),
				new org.springframework.data.valkey.connection.Limit().offset(Long.valueOf(offset).intValue())
