 ...
 * @author Zhian Chen
 * @since 2.0
 * @see org.springframework.data.valkey.connection.ValkeyStandaloneConfiguration
 * @see org.springframework.data.valkey.connection.ValkeySentinelConfiguration
 * @see org.springframework.data.valkey.connection.ValkeyClusterConfiguration
 */
public interface LettuceClientConfiguration {
	...
	class LettuceClientConfigurationBuilder {
		...
		/**
		 * Configure a {@link ValkeyCredentialsProviderFactory} to obtain {@link io.lettuce.core.RedisCredentialsProvider}
		 * instances to support credential rotation.
		 *
		 * @param valkeyCredentialsProviderFactory must not be {@literal null}.
		 * @return {@literal this} builder.
		 * @throws IllegalArgumentException if valkeyCredentialsProviderFactory is {@literal null}.
		 * @since 3.0
		 */
		public LettuceClientConfigurationBuilder valkeyCredentialsProviderFactory(
				ValkeyCredentialsProviderFactory valkeyCredentialsProviderFactory) {

			Assert.notNull(valkeyCredentialsProviderFactory, "ValkeyCredentialsProviderFactory must not be null");

			this.valkeyCredentialsProviderFactory = valkeyCredentialsProviderFactory;
			return this;
		}
		...
		public LettuceClientConfiguration build() {
			...
			return new DefaultLettuceClientConfiguration(useSsl, verifyMode, startTls, clientResources, clientOptions,
					clientName, readFrom, valkeyCredentialsProviderFactory, timeout, shutdownTimeout, shutdownQuietPeriod);
		}
	}
