...
class EnabledOnValkeyDriverCondition implements ExecutionCondition {
	...
	public ConditionEvaluationResult evaluateExecutionCondition(ExtensionContext context) {
		...
		for (Field field : annotatedFields) {
			...
			for (ValkeyDriver valkeyDriver : annotation.value()) {
				if (valkeyDriver.matches(value)) {
					foundMatch = true;
				}
			}
	...
	private static String formatUnsupportedDriver(ValkeyConnectionFactory value) {
		...
		for (ValkeyDriver valkeyDriver : ValkeyDriver.values()) {
			if (valkeyDriver.matches(value)) {
				return valkeyDriver.toString();
			}
		}
