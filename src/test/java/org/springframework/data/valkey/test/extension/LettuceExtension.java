 ...
 *
 * 	private final ValkeyCommands&lt;String, String&gt; valkey;
 *
 * 	public CustomCommandTest(StatefulRedisConnection&lt;String, String&gt; connection) {
 * 		this.valkey = connection.sync();
 * 	}
 *
 * }
 * </pre>
 ...
 * through default {@link RedisClient} respective {@link ValkeyClientSupplier} so shutdown is managed by the actual
...
public class LettuceExtension implements ParameterResolver, AfterAllCallback, AfterEachCallback {
	...
	public void afterEach(ExtensionContext context) {
		...
		RedisClient valkeyClient = store.get(RedisClient.class, RedisClient.class);
		if (valkeyClient != null) {
			valkeyClient.setOptions(DEFAULT_OPTIONS);
		}
	}
	...
	record ValkeyClientCloseable(AbstractRedisClient client) implements Closeable {
	...
	enum StatefulValkeyConnectionSupplier implements Supplier<StatefulRedisConnection<String, String>> {
	...
	enum StatefulValkeyPubSubConnectionSupplier implements Supplier<StatefulRedisPubSubConnection<String, String>> {
	...
	enum StatefulValkeyClusterConnectionSupplier implements Supplier<StatefulRedisClusterConnection<String, String>> {
