...
import org.springframework.data.valkey.core.ExpireChanges.ExpiryChangeState;
import org.springframework.data.valkey.core.types.Expirations.TimeToLive;
...
public class DefaultHashOperationsIntegrationTests<K, HK, HV> {
	...
	void testExpireWithOptionsNone() {
		...
		ExpireChanges<Object> expire = valkeyTemplate.opsForHash().expire(key,
				org.springframework.data.valkey.core.types.Expiration.seconds(20), ExpirationOptions.none(), List.of(key1));
	...
	void testExpireWithOptions() {
		...
		valkeyTemplate.opsForHash().expire(key, org.springframework.data.valkey.core.types.Expiration.seconds(20),
				ExpirationOptions.none(), List.of(key1));
		valkeyTemplate.opsForHash().expire(key, org.springframework.data.valkey.core.types.Expiration.seconds(60),
				ExpirationOptions.none(), List.of(key2));

		ExpireChanges<Object> changes = valkeyTemplate.opsForHash().expire(key,
				org.springframework.data.valkey.core.types.Expiration.seconds(30), ExpirationOptions.builder().gt().build(),
				List.of(key1, key2));
