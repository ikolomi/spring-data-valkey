...
public class DefaultReactiveHashOperationsIntegrationTests<K, HK, HV> {
	...
	void testExpireWithOptions() {
		...
		hashOperations
				.expire(key, org.springframework.data.valkey.core.types.Expiration.seconds(20), ExpirationOptions.none(),
						List.of(key1))
				.as(StepVerifier::create)//
				.assertNext(changes -> {
					assertThat(changes.allOk()).isTrue();
		...
		hashOperations
				.expire(key, org.springframework.data.valkey.core.types.Expiration.seconds(60), ExpirationOptions.none(),
						List.of(key2))
				.as(StepVerifier::create)//
				.assertNext(changes -> {
					assertThat(changes.allOk()).isTrue();
		...
		hashOperations
				.expire(key, org.springframework.data.valkey.core.types.Expiration.seconds(30),
