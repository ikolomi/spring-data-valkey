...
import org.springframework.data.valkey.connection.ReactiveListCommands;
import org.springframework.data.valkey.connection.ReactiveListCommands.LPosCommand;
import org.springframework.data.valkey.connection.ReactiveListCommands.PopResult;
import org.springframework.data.valkey.connection.ReactiveListCommands.PushCommand;
...
public class LettuceReactiveListCommandIntegrationTests extends LettuceReactiveCommandsTestSupport {
	...
	@ParameterizedValkeyTest // DATAVALKEY-852
	...
	void lTrimShouldReturnValuesCorrectlyWithMinUnbounded() {
		...
		connection.listCommands().lTrim(Mono.just(rangeCommand)).as(StepVerifier::create) //
				.expectNext(new ReactiveValkeyConnection.BooleanResponse<>(rangeCommand, true)) //
				.verifyComplete();
	}
