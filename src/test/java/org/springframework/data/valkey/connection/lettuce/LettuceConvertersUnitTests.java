...
import static org.springframework.data.valkey.connection.lettuce.LettuceCommandArgsComparator.*;
...
class LettuceConvertersUnitTests {
	...
	void toLimit() {
		...
		Limit limit = LettuceConverters.toLimit(org.springframework.data.valkey.connection.Limit.unlimited());
		assertThat(limit.isLimited()).isFalse();
		assertThat(limit.getCount()).isEqualTo(-1L);

		limit = LettuceConverters.toLimit(org.springframework.data.valkey.connection.Limit.limit().count(-1));
		assertThat(limit.isLimited()).isTrue();
		assertThat(limit.getCount()).isEqualTo(-1L);

		limit = LettuceConverters.toLimit(org.springframework.data.valkey.connection.Limit.limit().count(5));
		assertThat(limit.isLimited()).isTrue();
